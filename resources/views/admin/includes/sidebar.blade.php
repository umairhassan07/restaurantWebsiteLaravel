  <!-- partial:partials/_sidebar.html -->
  <nav class="sidebar sidebar-offcanvas" id="sidebar">
    <ul class="nav">
      <li class="nav-item nav-category">Main</li>
      <li class="nav-item">
        <a class="nav-link" href="{{ route('dashboard') }}">
          <span class="icon-bg"><i class="mdi mdi-cube menu-icon"></i></span>
          <span class="menu-title">Dashboard</span>
        </a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="{{ route('sliders') }}">
          <span class="icon-bg"><i class="mdi mdi-file-image"></i></span>
          <span class="menu-title">Sliders</span>
        </a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="{{ route('our-menu') }}">
          <span class="icon-bg"><i class="mdi mdi-book-open-page-variant"></i></span>
          <span class="menu-title">Our Menu</span>
        </a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="{{ route('our-cheff') }}">
          <span class="icon-bg"><i class="mdi mdi-account-multiple-outline"></i></span>
          <span class="menu-title">Our CHEFS</span>
        </a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="{{ route('reservations') }}">
          <span class="icon-bg"><i class="mdi mdi-alarm-check"></i></span>
          <span class="menu-title">Reservations</span>
        </a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="{{ route('meals') }}">
          <span class="icon-bg"><i class="mdi mdi-food-variant"></i></span>
          <span class="menu-title">Meals</span>
        </a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="{{ route('about-us') }}">
          <span class="icon-bg"><i class="mdi mdi-google-pages"></i></span>
          <span class="menu-title">About Us</span>
        </a>
      </li>
      <li class="nav-item">
        <a class="nav-link" href="{{ route('contact') }}">
          <span class="icon-bg"><i class="mdi mdi-contact-mail"></i></span>
          <span class="menu-title">Contact us</span>
        </a>
      </li>
    </ul>
  </nav>
  <!-- partial -->